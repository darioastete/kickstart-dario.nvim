...
-- Exit file
vim.keymap.set('n', '<leader>q', '<cmd>q<cr>', { desc = 'Cerrar archivo' })

-- En modo visual: tab y shift-tab para identar/desindentar
...
vim.keymap.set('n', '<leader>go', function()
  generate_commit_with_shortcut 'commit_obsidian'
end, { desc = 'Generate commit from obsidian project' })
-- SPLIT CODE PAGE
...
require('lazy').setup({
  ...
  { 'rasulomaroff/reactive.nvim' },
  {
    ...
    keys = {
      ...
      { '<leader>fk', '<cmd>FzfLua keymaps<cr>', desc = '[S]earch [K]eymaps' },
      { '<leader>fd', '<cmd>FzfLua diagnostics<cr>', desc = '[S]earch [D]iagnostics' },
      { '<leader>fr', '<cmd>FzfLua resume<cr>', desc = '[S]earch [R]esume' },
      { '<leader>fo', '<cmd>FzfLua oldfiles<cr>', desc = '[S]earch Recent Files' },
      { '<leader><leader>', '<cmd>FzfLua buffers<cr>', desc = '[ ] Find Buffers' },
      {
        '<leader>f/',
        function()
          require('fzf-lua').blines { previewer = false, winopts = { winblend = 10 } }
        end,
